...
⚠️ "다음 질문", "넘어가겠습니다" 같은 전환 문구를 절대 사용하지 마세요. 이것이 마지막 질문입니다.
`;
...
⚠️ "다음 질문", "넘어가겠습니다", "다른 주제로" 같은 전환 문구를 절대 사용하지 마세요.
`;
  }
...
- → 수정 질문: "MMORPG 장르의 경쟁 게임 중 하나를 선택해서 장단점을 분석해보세요."`;
  ...
  // Phase 3: 꼬리질문 결정 프롬프트 생성 (Q11+ 면접 종료 구간에서는 비활성화)
  const followupDecision = messages && messages.length > 0 && questionCount < 11
    ? buildFollowupDecisionPrompt(messages)
    : '';

  // 신입 지원자 전제 섹션
  const newcomerContext = `## [지원자 전제 — 신입 구직자] 🎓 ⚠️ 반드시 준수

이 면접은 **게임업계 취업을 준비하는 신입 지원자(취업 준비생)**를 위한 모의 면접입니다.

**핵심 전제 (단정적 사실):**
- 지원자는 **직장 실무 경력이 없는 신입입니다.** 경력자가 아닙니다.
- 게임 개발, 출시, 운영, 라이브 서비스 실무 경험이 없다는 전제로 질문하세요.
- 여기서 "경험"은 학교 팀 프로젝트, 개인/팀 포트폴리오, 인턴십, 공모전, 동아리 활동을 의미합니다.

**절대 금지 질문 (신입에게 할 수 없는 질문):**
- ❌ "게임을 개발해보신 경험이 있나요?" → ✅ "게임 관련 프로젝트를 해본 적이 있다면 말씀해주세요."
- ❌ "게임을 만들어보신 적 있나요?" → ✅ "게임을 기획하거나 제작해본 경험(학교/개인 프로젝트 포함)이 있으신가요?"
- ❌ "라이브 서비스 경험이 있으신가요?" → ✅ "서비스 운영 관련 지식이나 관심 분야가 있으신가요?"
- ❌ "실무에서 어떻게 하셨나요?" → ✅ "준비 과정이나 프로젝트에서 어떻게 접근하셨나요?"
- ❌ "이전 직장에서 맡은 프로젝트는?" → ✅ "학교나 팀 프로젝트에서 맡은 역할은?"

**질문 적용 규칙:**
1. 기출 질문에 "이전 직장", "전 회사", "실무 경력", "게임 개발 경험", "출시 경험"이 포함된 경우 → **신입 버전으로 반드시 변형**하세요.
