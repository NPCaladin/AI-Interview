...
export const CONTEXTUAL_PATTERNS: Array<{
  ...
  {
    category: '실무 경력 가정',
    description: '직장 실무 경력이 있음을 전제 (신입에게 부적절)',
    patterns: [
      /실무에서\s*(어떻게|담당|경험|해보|하셨)/,
      /현장에서\s*(어떻게|담당|경험|해보|하셨)/,
      /재직\s*(중에|하면서|당시|시절)/,
      /경력을\s*(바탕으로|토대로|살려)/,
      /업무\s*경력\s*(이|을|은|가)/,
      /(해보셨|해보신|해본\s*적|하신\s*경험)\s*(이|이\s*있|은|을)/,
    ],
  },
  {
    category: '게임 개발 경험 가정',
    description: '게임 개발/출시/운영 실무 경험이 있음을 전제 (신입에게 부적절)',
    patterns: [
      /게임.*(개발|만들어|출시|서비스).*(경험|해보|하신|있으신)/,
      /게임을\s*(만들어|개발해|출시해)\s*(보신|보셨|본\s*적)/,
      /라이브\s*서비스\s*(경험|해보|하신)/,
      /(개발|운영|서비스)해\s*보신/,
      /직접\s*(개발|출시|운영|서비스)해\s*(보신|보셨|본\s*적)/,
    ],
  },
];
...
10. 실무 경력 가정 — "실무에서 담당하셨던...", "현장에서 해보셨나요?", "재직 중에..." (지원자가 실무 경력을 직접 언급하지 않은 경우)
